...
describe('viper service', () => {
  ...
  describe('viper rest service errors', () => {
    ...
    it('rejects if it receives an invalid body (bad rating)', () => {
      ...
      return viperService.rating('A123')
        .catch((error) => {
          expect(error.message).to.equal('Invalid body: {"nomsId":"A123","viperRating":-1}');
        });
    });
    ...
    it('rejects if it receives an invalid body (no rating)', () => {
      ...
      return viperService.rating('A123')
        .catch((error) => {
          expect(error.message).to.equal('Invalid body: {"nomsId":"A123","age":21}');
        });
    });
    ...
    it('rejects if it receives an invalid body (no nomsId)', () => {
      ...
      return viperService.rating('A123')
        .catch((error) => {
          expect(error.message).to.equal('Invalid body: {"id":"A123","viperRating":0.99}');
        });
    });
    ...
    it('rejects if it receives an invalid body (totally wrong)', () => {
      ...
      return viperService.rating('A123')
        .catch((error) => {
          expect(error.message).to.equal('Invalid body: <nomsId>A123</nomsId><viperRating>0.34</viperRating>');
        });
    });
    ...
    it('rejects if it receives an empty body error', () => {
      ...
      return viperService.rating('A123')
        .catch((error) => {
          expect(error.message).to.equal('Invalid body: ');
        });
    });
  });
});
