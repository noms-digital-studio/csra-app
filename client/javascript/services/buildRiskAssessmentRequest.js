...
const extractReasons = (questions, answers) => {
  ...
  const reasons = questionsWithPredicates.reduce((reasonsList, question) => {
    ...
    if (answers[question.section].answer === 'yes' && not(isEmpty(question.sharedCellPredicate.reasons))) {
      const reason = {
...
const buildRiskAssessmentRequest = ({ nomisId, outcome, viperScore, questions, answers }) => ({
  nomisId,
...
export default buildRiskAssessmentRequest;
